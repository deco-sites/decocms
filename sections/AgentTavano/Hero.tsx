...
export default function Hero({
  ...
  description =
    "O Agent Tavano é um assistente de IA especializado que revoluciona a forma como você cria e gerencia sua loja online.",
  ...
  return (
    <section class="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-primary via-primary/95 to-primary/90">
      ...
      <div class="absolute inset-0 overflow-hidden">
        <div class="absolute -top-40 -right-40 w-80 h-80 bg-white/5 rounded-full blur-3xl animate-pulse">
        </div>
        <div class="absolute -bottom-40 -left-40 w-96 h-96 bg-white/3 rounded-full blur-3xl animate-pulse delay-1000">
        </div>
        <div class="absolute top-1/2 left-1/4 w-64 h-64 bg-white/5 rounded-full blur-2xl animate-pulse delay-500">
        </div>
      </div>
      ...
      <div class="relative z-10 max-w-6xl mx-auto px-4 text-center text-white">
        ...
        <h1 class="text-5xl md:text-6xl lg:text-7xl font-bold mb-6 leading-tight tracking-tight">
          {title.split("\n").map((line, index) => <div key={index}>{line}
          </div>)}
        </h1>
        ...
        <div class="flex justify-center items-center space-x-8 text-white/60 text-sm">
          ...
          <div class="flex items-center space-x-2">
            <div class="w-2 h-2 bg-blue-400 rounded-full animate-pulse delay-300">
            </div>
            <span>Seguro & Confiável</span>
          </div>
          <div class="flex items-center space-x-2">
            <div class="w-2 h-2 bg-purple-400 rounded-full animate-pulse delay-600">
            </div>
