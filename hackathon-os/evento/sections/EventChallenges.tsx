...
export default function EventChallenges({ eventId }: EventChallengesProps) {
  ...
  const [challenges, setChallenges] = useState(
    mockChallenges.filter((c) => c.eventId === eventId),
  );
  ...
  const canSeePending = currentUser?.role === "admin" ||
    currentUser?.role === "organizador";

  const handleVote = (challengeId: string) => {
    setChallenges((prev) =>
      prev.map((c) => c.id === challengeId ? { ...c, votes: c.votes + 1 } : c)
    );
  };

  const handleApprove = (challengeId: string) => {
    setChallenges((prev) =>
      prev.map((c) => c.id === challengeId ? { ...c, approved: true } : c)
    );
  ...
  return (
    <div class="space-y-6">
      ...
      <div>
        ...
        <div class="grid md:grid-cols-2 gap-4">
          {approvedChallenges.map((challenge) => (
            <div
              key={challenge.id}
              class="bg-gradient-to-br from-purple-50 to-pink-50 border-2 border-purple-200 rounded-lg p-6"
            >
      ...
      {event.config.challengesVotingPublic && !canSeePending &&
        pendingChallenges.length > 0 && (
        <div>
          <h3 class="text-xl font-semibold text-gray-900 mb-4">
            Desafios Propostos pela Comunidade
          </h3>
          <div class="grid md:grid-cols-2 gap-4">
            {pendingChallenges.map((challenge) => (
              <div
