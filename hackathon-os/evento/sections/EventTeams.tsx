...
import { getEventById, getEventTeams, getUserById } from "../../data/mockData.ts";
...
export default function EventTeams({ eventId }: EventTeamsProps) {
  ...
  const userTeam = teams.find(t => t.memberIds.includes(currentUser?.id || ""));
    ...
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-gray-600 text-sm">Máximo de {event.config.maxTeamSize} membros por time</p>
        </div>
      ...
      {teams.length === 0 ? (
        ...
        <div class="grid md:grid-cols-2 gap-6">
          {teams.map(team => {
            ...
            const isUserInTeam = team.memberIds.includes(currentUser?.id || "");

            return (
              <div 
                key={team.id} 
                class={`bg-white rounded-lg shadow-md p-6 ${isUserInTeam ? "ring-2 ring-purple-500" : ""}`}
              >
                {isUserInTeam && (
                  <div class="mb-3">
                        ...
                        <div class="flex-1 min-w-0">
                          <div class="text-sm font-medium text-gray-900 truncate">
                            ...
                            {member.id === team.leaderId && (
                              <span class="ml-2 text-xs text-purple-600">👑 Líder</span>
                            )}
                          </div>
                          <div class="text-xs text-gray-500 capitalize">{member.area}</div>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {team.challengeId && (
                  <div class="pt-3 border-t border-gray-200">
                    <div class="text-xs font-semibold text-gray-500 uppercase mb-1">Desafio</div>
