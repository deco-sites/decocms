...
const data = JSON.parse(
  fs.readFileSync(".deco/blocks/pages-My%2520New%2520Page-529958.json", "utf8"),
);

// Read the extracted cards
const extractedCards = JSON.parse(
  fs.readFileSync("extracted_cards.json", "utf8"),
);

// Find the ProjectGrid section and update it properly
for (const section of data.sections) {
  if (
    section.__resolveType === "website/sections/Rendering/Lazy.tsx" &&
    ...
    if (
      section.section.layoutCategoryCard &&
      Object.keys(section.section.layoutCategoryCard).length === 0
    ) {
      delete section.section.layoutCategoryCard;
    }

    console.log(
      "Fixed ProjectGrid structure with",
      extractedCards.length,
      "direct project cards",
    );
    break;
  }
}
...
fs.writeFileSync(
  ".deco/blocks/pages-My%2520New%2520Page-529958.json",
